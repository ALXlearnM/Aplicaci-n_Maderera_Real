using System;
using System.Collections.Generic;

namespace Maderera_Aplicacion_Web.Models
{
    public partial class Sntt26ModalidadFormativa
    {
        public Sntt26ModalidadFormativa()
        {
            Pert04Empleados = new HashSet<Pert04Empleado>();
        }

        public int IdModalidadFormativa { get; set; }
        public string? CodModalidadFormativa { get; set; }
        public string? CodModalidadFormativaPle { get; set; }
        public string? TxtAbrv { get; set; }
        public string? TxtDesc { get; set; }
        public int? IdEstado { get; set; }
        public string? TxtEstado { get; set; }

        public virtual ICollection<Pert04Empleado> Pert04Empleados { get; set; }
    }
}
